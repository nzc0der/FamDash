...
def init_schema() -> None:
    ...
    with _get_db() as conn:
        ...
        try:
            conn.execute("ALTER TABLE events ADD COLUMN color TEXT;")
        except sqlite3.OperationalError:
            pass
...
def get_announcement(ann_id: int) -> dict | None:
    """Return a single announcement by id, or None if it does not exist."""
    with _get_db() as conn:
        row = conn.execute(
            """
            SELECT a.id, a.title, a.body, a.pinned, a.created_at, a.updated_at,
                   a.author_id, u.username AS author
              FROM announcements a
              LEFT JOIN users u ON u.id = a.author_id
             WHERE a.id = ?
            """,
...
def save_push_subscription(user_id: int, endpoint: str, p256dh: str, auth: str) -> int:
    """Insert or update a push subscription for a user, keyed by endpoint."""
    ...
    with _get_db() as conn:
        ...
        conn.execute(
            ...
            UPDATE push_subscriptions
               SET user_id = ?, p256dh = ?, auth = ?, created_at = CURRENT_TIMESTAMP
             WHERE id = ?
            """,
            (user_id, p256dh, auth, existing["id"]),
        )
...
def delete_push_subscription(subscription_id: int) -> None:
    """Remove a stored push subscription by id."""
    with _get_db() as conn:
...
def get_all_events() -> list[dict]:
    ...
    with _get_db() as conn:
        rows = conn.execute(
            """
            SELECT e.id, e.title, e.event_date, e.end_date, e.author_id, e.color, u.username as author
            FROM events e
            LEFT JOIN users u ON u.id = e.author_id
            ORDER BY e.event_date ASC
            """
...
def add_event(author_id: int, title: str, event_date: str, end_date: str = None, color: str = None) -> int:
    """Insert a new event and return its id."""
    with _get_db() as conn:
        cur = conn.execute(
            "INSERT INTO events (author_id, title, event_date, end_date, color) VALUES (?, ?, ?, ?, ?)",
            (author_id, title.strip(), event_date.strip(), end_date.strip() if end_date else None, color.strip() if color else None),
        )
        return cur.lastrowid
